...
function cosine(v, w)
  return torch.Tensor({v:dot(w) / v:norm() / w:norm()})
end
...
feval = function(x_new)
   ...
   forwardInputs = {cosine(model:forward(inputs),target),cosine(inputs,target)}
    backwardInputs = {cosine(model.output,target),cosine(inputs,target)}
    print(forwardInputs)
    print(forwardInputs[1])
    print(forwardInputs[2])
   local loss_x = criterion:forward(forwardInputs,1)
    print(loss_x)
    p =  criterion:backward(backwardInputs,1)
    print(p)
    print(table.getn(p))
   --model:backward(inputs, criterion:backward({model.output, target},1)[1])
   model:backward(inputs,p)
